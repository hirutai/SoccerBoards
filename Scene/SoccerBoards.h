...
private: // フィールドプレイヤー
	// 描画用
	std::unique_ptr<Obj3dModel> modelMyBody; // モデル
	std::unique_ptr<Obj3dObject> objectMyBody[PLAYERNUM]; // 3Dオブジェクト
...
private: // ゴールキーパー
	// 描画用
	std::unique_ptr<Obj3dModel> modelMyKeaper; // モデル
	std::unique_ptr<Obj3dObject> objectMyKeaper; // 3Dオブジェクト
...
private: // フィールドプレイヤー
	// 全体描画用
	std::unique_ptr<Obj3dModel> modelCpuBody; // モデル
	std::unique_ptr<Obj3dObject> objectCpuBody[PLAYERNUM]; // 3Dオブジェクト
...
private: // ゴールキーパー
	std::unique_ptr<Obj3dModel> modelCpuKeaper; // モデル
	std::unique_ptr<Obj3dObject> objectCpuKeaper; // 3Dオブジェクト
...
#pragma region タイトル画像
	std::unique_ptr<Sprite> title; // タイトル
#pragma endregion

#pragma region 操作説明
	std::unique_ptr<Sprite> ex; // 操作説明
#pragma endregion
...
#define LEFTEND -WindowsApi::windowWidth / 2 // 左端
	...
	std::unique_ptr<Sprite> kickoff; // キックオフの文字
...
#define VICTORYSCORE 3 // 勝利スコア
	...
	std::unique_ptr<Sprite> myScoreNumber; // 味方得点用数字
	std::unique_ptr<Sprite> cpuScoreNumber; // 相手得点用数字
...
#define EXPRATE 0.01f // 拡大率
	...
	std::unique_ptr<Sprite> goalCha; // ゴール！の文字
...
#pragma endregion 試合結果
	std::unique_ptr<Sprite> result; // 試合結果
#pragma region 
...
private: // メンバ
	...
	// 配列へ衝突形状の追加
	void PushCollisionShape(btCollisionShape* collsionShape);
